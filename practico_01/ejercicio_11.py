...
def suma_cubo_pares_sum_list(numeros: Iterable[int]) -> int:
    ...
    """
    return sum([n**3 for n in filter(lambda n: n%2==0, numeros)])
...
suma_numeros_al_cubo_pares = reduce(lambda n, m: n+m, numeros_al_cubo_pares)


# Escribir una función Lambda que permita ordenar los elementos de la numeros
# en base a si son pares o impares

numeros_ordenada = sorted(numeros, key=(lambda n: n%2), reverse=True)
